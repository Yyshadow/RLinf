...
export RLINF_PI05_NORM_STATS_PATH="${RLINF_PI05_NORM_STATS_PATH:-${RLINF_PI05_DATA_ROOT}/beat_block_hammer_success_50_train/norm_stats.json}"
...
case "${MODE}" in
    smoke)
        EXPERIMENT_NAME="pi05_hammer50_full_smoke10"
        MAX_STEPS=10
        SAVE_INTERVAL=10
        MICRO_BATCH_SIZE=1
        GLOBAL_BATCH_SIZE=8
...
if [ ! -f "${TOKENIZER_PATH}" ]; then
    echo "Missing OpenPI tokenizer cache: ${TOKENIZER_PATH}" >&2
    exit 1
fi
